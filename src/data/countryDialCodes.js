/** International dialing codes. UK (+44) is first for default selection. */
...
export function formatIntlPhone(dial, localNumber) {
  const digits = String(localNumber || '').replace(/\D/g, '')
  const code = String(dial || DEFAULT_DIAL_CODE).replace(/\D/g, '')
  if (!digits) return ''
  return `+${code}${digits}`
}
